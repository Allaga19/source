...
window.addEventListener('DOMContentLoaded', function () {
  ...
  Object(_modules_modals__WEBPACK_IMPORTED_MODULE_0__["default"])(); // sliders('.feedback-slider-item', '', '.main-prev-btn', '.main-next-btn');
  ...
  Object(_modules_pictureSize__WEBPACK_IMPORTED_MODULE_8__["default"])('.sizes-block'); // accordion('.accordion-heading', '.accordion-block'); /*для 1 варианта*/ */
...
var accordion = function accordion(triggersSelector) {
  ...
  var btns = document.querySelectorAll(triggersSelector); // 1 вариант
...
var calc = function calc(size, material, options, promocode, result) {
  ...
  var sum = 0; // функция которая будет срабатывать каждый раз,
  // когда ползователь делает выбор.
...
var forms = function forms() {
  var form = document.querySelectorAll('form'),
      inputs = document.querySelectorAll('input'); //создаём объект с сообщениями
  ...
  form.forEach(function (item) {
    item.addEventListener('submit', function (e) {
      ...
      statusMessage.classList.add('status'); // помещаем дополнительный блок statusMessage в родителя этой формы

      item.parentNode.appendChild(statusMessage); // добавляем класс анимации

      item.classList.add('animated', 'fadeOutUp'); // скрываем форму

      setTimeout(function () {
        item.style.display = "none";
      }, 400); // отображение статуса сообщения спинер
      // создаём переменную для спинера

      var statusImg = document.createElement('img'); // прописываем отрибуты спинеру

      statusImg.setAttribute('src', message.spinner); // анимация для спинера

      statusImg.classList.add('animated', 'fadeInUp'); // вывод спинера на страницу

      statusMessage.appendChild(statusImg); // создаём переменную для текста
      ...
      Object(_services_requests__WEBPACK_IMPORTED_MODULE_4__["postData"])(api, formData).then(function (res) {
        console.log(res); //изменение изображения

        statusImg.setAttribute('src', message.ok); // если успешно ушёл запрос
      ...
      .finally(function () {
        ...
        clearInputs(); // устанавливаем анонимный таймаут

        setTimeout(function () {
          statusMessage.remove(); // после того как удалится изображение

          item.style.display = 'block'; // удаляем класс скрытия формы

          item.classList.remove('fadeOutUp'); // добавляем клас кросивого появления формы
...
var mask = function mask(selector) {
  ...
  var setCursorPosition = function setCursorPosition(pos, elem) {
    elem.focus(); // для старых браузеров
  ...
  function createMask(event) {
    ...
    val = this.value.replace(/\D/g, ''); // запрет на удаление +7
...
var modals = function modals() {
  ...
  function bindModal(triggerSelector, modalSelector, closeSelector) {
    ...
    close.addEventListener('click', function () {
      console.log(close); // когда нажимаем крестик, то закрываем все модальные окна
...
var pictureSize = function pictureSize(imgSelector) {
  var blocks = document.querySelectorAll(imgSelector); // мышка входит в пределы блока

  function showImg(block) {
    var img = block.querySelector('img'); // something.png => something-1.png

    img.src = img.src.slice(0, -4) + '-1.png'; // скрываем все парагрофы внутри блока
...
var showMoreStyles = function showMoreStyles(trigger, wrapper) {
  var btn = document.querySelector(trigger); // проверим работает ли код
  ...
  function createCards(response) {
    response.forEach(function (_ref) {
      ...
      card.classList.add('animated', 'fadeInUp', 'col-sm-3', 'col-sm-offset-0', 'col-xs-10', 'col-xs-offset-1'); // наполнение карточки
...
var sliders = function sliders(slides, dir, prev, next) {
  ...
  var slideIndex = 1,
      paused = false; // элементы скоторыми будем работать

  var items = document.querySelectorAll(slides); // функция, которая отвечает за перемещение слайд-индекса (n) и слайдера
  ...
  function showSlides(n) {
    ...
    items.forEach(function (item) {
      ...
      item.classList.add("animated"); // скрываем все слайды
  ...
  } catch (e) {} // активация анимации и интервала
  ...
  activateAnimation(); // отключение автоиатического переключения слайдеров
